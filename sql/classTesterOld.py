...
serviceNegComb = ["out of coverage","out of range","too much interrupt", "recharge not successful", "not connect","not connecting", "no internet connection", "no service", "piling the bill"];
...
satisfactionNegComb = ["never opt for", "time to port","do number portability"];
...
serviceAspect = ["network","speed","call","3g","2g","4g","data pack","gprs","internet","net","wimax","download", "downloaded", "downloading", "coverage","browse","msg","sms","message","delivery","recharge","bill","billing"]; 
#better, download
...
miscAspect    = ['customer care', 'customer service centre', 'offer', 'pack', 'package', 'plan', 'server', 'service provider', 'tariff', 'web page', 'web site', 'website'];

satisfactionAspect = ["change","changed","changing","port","ported","porting","activated","churning","churned","switch","switched","switching", "mnped", "transfer"];
...
negWithRegExp = open('negWithRegExp.txt','a+');
trigramNegNeg = open('trigramNegNeg.txt','a+');
		...
		if pos2 != -1:
			...
			for word1 in posWordsB:
				...
				if pos1 != -1 and pos2 > pos1:
					#~ wordDist = string.count(tweet," ",pos1,pos2);
					#~ if wordDist <= 3:
					...
					for word3 in other_service_provider:
						...
						if pos3 != -1 and pos3 > pos2:
							if isNeg == []: #Negation does not occur in the tweet
								...
								negative_score += 1;			
								trigramNegNeg.write("n\t"+tweet+"\n");
	return aspect, positive_score, negative_score											

def uniuniTester(tweet, aspectList, posWordList, negWordList,isNeg):
	positive_score = 0;		negative_score=0;
...
def classifier(tweet, service_provider):
	...
	if neutral_score != 1:				
		...
		isPrice,   positive_scoreP, negative_scoreP = bigramTester(tweet, priceAspect, isPrice, pricePosVerb+POSWORDS, priceNegVerb+NEGWORDS, pricePosVerb+POSWORDS, priceNegVerb+NEGWORDS, isNeg);
		
		#Check for service aspect
		isService1, positive_scoreS1, negative_scoreS1 = bigramTester(tweet, serviceAspect, isService, servicePosClass1+POSWORDS, serviceNegClass1+NEGWORDS, servicePosClass2+POSWORDS, serviceNegClass2+NEGWORDS, isNeg);
		...
		isSatis,  positive_scoreSa2, negative_scoreSa2 = wordCombMatcher(tweet, isSatis1, satisfactionPosComb, satisfactionNegComb, isNeg);
		...
		isMisc2, positive_scoreM2, negative_scoreM2 = bigramTester(tweet, miscAspect, isMisc1, miscPosClass1+POSWORDS, miscNegClass1+NEGWORDS, miscPosClass2+POSWORDS, miscNegClass2+NEGWORDS, isNeg);
		...
		positive_scoreG, negative_scoreG = uniuniTester(tweet, priceAspect+serviceAspect+satisfactionAspect+miscAspect, POSWORDS, NEGWORDS, isNeg);
		
		#Accumlated positive or negative score
		positive_score = positive_scoreP + positive_scoreS1 + positive_scoreS2 + positive_scoreSa1 + positive_scoreSa2 + positive_scoreM + positive_scoreM1 + positive_scoreM2 + positive_scoreG;
		negative_score = negative_scoreP + negative_scoreS1 + negative_scoreS2 + negative_scoreSa1 + negative_scoreSa2 + negative_scoreM + negative_scoreM1 + negative_scoreM2 + negative_scoreG;
		
	return positive_score, negative_score, neutral_score, isPrice, isService, isSatis, isMisc

str1 = "dad pissed off at bsnl another service activated without consent thats it instant message port the number"
