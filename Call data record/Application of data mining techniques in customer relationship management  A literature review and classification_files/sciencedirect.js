...
s.trackingServerSecure = "smetrics.sciencedirect.com"
s.trackingServer = "metrics.sciencedirect.com"
...
function pdfEventHandler(e) {
	if($(this).not(e.data).length == 0) return;
	var re = /pii[=|\/]([\w]+)(?=[&|\/pdf])/;
	var pii = re.exec(this.href);
	if(pii) {
		re = /zone=([\w]+)(?=[&])/;
		var zone = re.exec(this.href);
		...
		s.products=";"+pii[1];
		s.eVar17="pdf";
		s.events="prodView,event5,event27";
		s.prop1="";
		s.prop2=s.prop1;
		s.prop4="article";
		if(zone) s.eVar52=zone[1];
		s.t();
	}
}
$.expr[':'].pdf = function(e) {
...
var p = $("a:pdf").on("click",pdfEventHandler);
$(document).on("click","a:pdf",p,pdfEventHandler);
...
if(s.pageName && s.pageName!=""){
	...
	*/
	...
	/* Order Number: populate s.eVar11 with s.purchaseID */
	if(s.purchaseID&&!s.eVar22) s.eVar22=s.purchaseID;
	s.eVar22=s.getValOnce(s.eVar22,'ev22',0);
...
{x:{h:["www.sciencedirect.com"],p:["/activateaccess"]},
p:[{t:'c',c:'s.channel="home";\ns.pageName="remote access";\ns.prop1="home";\ns.prop2=s.prop1;\ns.prop4="authentication";'}]},
